-- THS-42 (E2.2) — Weekly G-score compute job.
-- Saturday 22:15 UTC, 15 min after Q so the per-factor jobs land in order
-- and don't race on the scores_history row.
...
BEGIN
  ...
  PERFORM cron.schedule(
    'compute-g-scores-weekly',
    '15 22 * * 6',
    $cron$
      SELECT net.http_post(
        url := (SELECT decrypted_secret FROM vault.decrypted_secrets WHERE name = 'project_url')
               || '/functions/v1/compute-g-scores',
