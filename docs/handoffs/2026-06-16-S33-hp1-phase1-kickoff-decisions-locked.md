...
- **Epics NOT created** — Linear MCP **writes are approval-gated this session** (the `save_issue` for the Phase 0 epic returned "requires approval"). The next session (with write-approval) should create 5 epics under the HP-1 project:
  ...
  2. **Phase 1 — Fork + data** → Urgent. Use the delta + fork-execution-order above. Sub-issues: fork+prune · rebrand chrome · `hp1.*` schema + stub alert call · rewire loaders · AIQ+Settings slice · add 19 tickers `kind='hp1'` + backfill · engine GH Action.
